...
def test_evaluate_candidate_waives_same_corpus_row_gate_for_one_gain_no_drift(
    tmp_path: Path,
):
    manager = PredictiveTrainerManager(_make_config(tmp_path))
    active = {
...
def test_evaluate_candidate_keeps_same_corpus_row_gate_without_quality_gain(
    tmp_path: Path,
):
    manager = PredictiveTrainerManager(_make_config(tmp_path))
    active = {
...
def test_evaluate_candidate_waives_bounded_representative_live_row_gate(
    tmp_path: Path,
):
    manager = PredictiveTrainerManager(_make_config(tmp_path))
    active = {
        "training_rows": 71_361,
        "shadow_rows": 70_975,
        "positive_rows": 46_459,
        "negative_rows": 24_902,
        "calibration_global_mae_sol": 0.0055025894001330235,
        "p_positive_after_cost_brier": 0.14995488094947076,
        "shadow_corpus_instance_id": "runtime_analytics:/Volumes/wd_black/runtime.noindex/algotrader/runtime_analytics.sqlite",
        "raw_shadow_entry_count": 268_662,
        "shadow_corpus_last_seq": 1_780_560_127_406,
    }
    candidate_model = {
        "calibration": {
            "global_mae_sol": 0.00556872201994025,
            "tradeability_head_brier": {"p_positive_after_cost": 0.14944504767942696},
        }
    }
    candidate_attestation = {
        "training": {
            ...
            "shadow_rows": 6_593,
            "shadow_corpus_instance_id": "runtime_analytics:/Volumes/wd_black/runtime.noindex/algotrader/runtime_analytics.sqlite",
            "raw_shadow_entry_count": 270_589,
            "shadow_corpus_last_seq": 1_780_566_467_063,
        },
        "validation": {"positive_rows": 4_220, "negative_rows": 2_760},
    }
...
def test_scheduler_trigger_does_not_retry_gated_off_run_below_min_new_rows(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    config = _make_config(tmp_path)
    manager = PredictiveTrainerManager(config)
    ...
    assert trigger["should_start"] is False
    assert trigger["requested_by"] is None
    ...
    assert trigger["gated_off_run_new_shadow_rows"] == 25
    assert trigger["gated_off_run_min_new_rows_reached"] is False
    assert trigger["last_run_promotion_state"] == "gated_off"
    assert trigger["new_shadow_rows_since_effective_baseline"] == 25


def test_scheduler_trigger_retries_gated_off_run_after_min_new_rows(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    config = _make_config(tmp_path)
    manager = PredictiveTrainerManager(config)
    ...
    manager._write_scheduler_state(
        {
            ...
            "last_run_started_at": "2026-04-07T00:00:00Z",
            "last_trigger_reason": "scheduler_row_threshold",
            "last_trigger_shadow_entry_count": 320,
        }
    )
    _write_shadow_sqlite_count(manager.config.shadow_sqlite_path, 325)
    manager._shadow_index_stats_cache = manager._refresh_shadow_index_stats_cache_sync()
    ...
    assert trigger["gated_off_run_has_new_rows"] is True
    assert trigger["gated_off_run_new_shadow_rows"] == 125
    assert trigger["gated_off_run_min_new_rows_reached"] is True
    assert trigger["last_run_promotion_state"] == "gated_off"
    assert trigger["new_shadow_rows_since_effective_baseline"] == 5
...
def test_scheduler_trigger_does_not_retry_gated_off_run_without_new_rows(
    ...
    assert trigger["gated_off_run_new_shadow_rows"] == 0
    assert trigger["gated_off_run_min_new_rows_reached"] is False
